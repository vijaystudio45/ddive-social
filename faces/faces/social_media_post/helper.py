...
import json
from authentication.models import GeneratePostPrompt
...
def generate_facebook_post(description,company_description,categoriesdata,prompt_text, channel):
    prompt_data = GeneratePostPrompt.objects.first()

    if prompt_data and hasattr(prompt_data, 'prompt'):

        prompts = prompt_data.prompt.format(channel=channel,description=description,prompt_text=prompt_text,company_description=company_description,categoriesdata=categoriesdata)

        # print(prompts,'------------------gegegegeg')


    # prompts = str(prompt_data.prompt).format(channel, description, prompt_text, company_description, categoriesdata)
